...
import { useKroYamlQuery } from './useKroYamlQuery';
...
describe('useKroYamlQuery', () => {
  ...
  it('skips the query when name/namespace is missing (forwarded to useYamlQuery)', () => {
    renderHook(() => useKroYamlQuery('', 'project-foo--ws-bar'));

    expect(useQueryMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ skip: true }));
  });

  it('extracts KroYaml from its own response shape', () => {
    useQueryMock.mockReturnValue({
      ...baseQueryResult,
      data: {
        kro_services_open_control_plane_io: {
          v1alpha1: { KroYaml: 'apiVersion: v1\nkind: Kro\n' },
        },
      },
    } as ReturnType<typeof useQuery>);

    const { result } = renderHook(() => useKroYamlQuery('my-cp', 'project-foo--ws-bar'));
