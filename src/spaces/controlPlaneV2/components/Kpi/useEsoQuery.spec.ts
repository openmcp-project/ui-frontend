...
describe('useEsoQuery', () => {
  ...
  it('reports a schema mismatch to telemetry and returns null', () => {
    // metadata.name is required by EsoSchema — omitting metadata triggers a validation failure
    useQueryMock.mockReturnValue(makeQueryResult({ spec: { version: '0.10.0' } }));
