...
export function ControlPlaneListWorkspaceGridTile({
  ...
  useEffect(() => {
    const settled = isReadyForSubscriptions || !!cpsError || isMember === false;
    if (!settled || hasFiredComplete.current) return;
    hasFiredComplete.current = true;
    onFetchComplete?.();
  }, [isReadyForSubscriptions, cpsError, isMember, onFetchComplete]);
  ...
  const hasMcpMatch =
    isReadyForSubscriptions && !isPending && query && !workspaceMatches && (visibleMcps ?? []).length > 0;
  // Hide tile when searching and nothing matches (workspace name/displayName or any CP name)
  const hidden = isReadyForSubscriptions && !isPending && query && !workspaceMatches && !hasMcpMatch;
        ...
        <section className={styles.workspaceSection} data-testid={`workspace-panel-${workspaceName}`}>
          ...
          {!shouldCollapsePanel && (
            <div className={styles.workspaceBody}>
              {errorView ? (
                errorView
              ) : !isReadyForSubscriptions || (isPending && managedControlPlanes?.length === 0) ? (
