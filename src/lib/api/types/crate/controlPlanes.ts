...
export const ControlPlane = (
  ...
  return {
    ...
    jq: '{ spec: .spec | {components, authorization: {roleBindings: .authorization.roleBindings}}, metadata: .metadata | {name, namespace, creationTimestamp, annotations}, status: { conditions: [.status.conditions[] | {type: .type, status: .status, message: .message, reason: .reason, lastTransitionTime: .lastTransitionTime}],  access: .status.components.authentication.access, status: .status.status }}',
  };
};
