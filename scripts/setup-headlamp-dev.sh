...
echo ""
echo "── Start the BFF with production CSP rules (no --local-dev) ────────────────"
echo ""
echo "  HEADLAMP_UPSTREAM_URL=http://localhost:${PORT} npx tsx ./server.ts"
echo ""
echo "  This runs the BFF without --local-dev, so NODE_ENV=production strict CSP"
echo "  applies (no 'unsafe-inline'). This matches the in-cluster behaviour and"
echo "  lets you verify CSP fixes locally before deploying."
echo ""
echo "  Make sure your .env has OIDC, API_BACKEND_URL etc. filled in."
echo "────────────────────────────────────────────────────────────────────────────"
